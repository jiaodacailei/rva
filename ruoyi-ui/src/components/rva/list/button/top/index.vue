  ...
  <div v-if="show">
    ...
    <template v-if="viewButtonData.action == 'dialog'">
      <template v-if="dialogSelector">
        <rva-crud-selector v-if="open" :open.sync="open" :appIds="[viewButtonData.actionDialogAppId]"
...
export default {
  ...
  computed: {
    buttonType() {
      return this.isPC ? this.viewButtonData.cls : 'text'
    }
  }
};
</script>
